 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_appCtrl_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_appCtrl_ClientHandlers,
    LE_MEM_BLOCKS(le_appCtrl_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_appCtrl_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_appCtrl_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_appCtrl_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appCtrl_SetDebug;
    _msgBufPtr = _msgPtr->buffer;
...
static void _Handle_ifgen_le_appCtrl_AddTraceAttachHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_appCtrl_TraceAttachHandlerFunc_t _handlerRef_ifgen_le_appCtrl_AddTraceAttachHandler =
        (le_appCtrl_TraceAttachHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_ifgen_le_appCtrl_AddTraceAttachHandler != NULL )
    {
        _handlerRef_ifgen_le_appCtrl_AddTraceAttachHandler(appRef, pid, procName, contextPtr );
    }
    else
    {
...
)
{
    ...
    _clientDataPtr->handlerPtr = attachToPidPtr;
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_appCtrl_TraceAttachHandlerRef_t)
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_appCtrl_AddTraceAttachHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_appCtrl_AddTraceAttachHandler, msgRef, clientDataPtr);
            break;
