 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_secStore_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_secStore_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_secStore_HasLocalBinding())
    {
        return NULL;
    }
    else
...
static void InitCommonData(void)
{
    ...
    ifgen_le_secStore_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_secStore_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
{
    return ifgen_le_secStore_Write(
...
{
    return ifgen_le_secStore_Delete(
        GetCurrentSessionRef(),
        name
    );
}
