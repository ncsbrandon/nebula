 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_temp_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_temp_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_temp_HasLocalBinding())
    {
        return NULL;
    }
    else
...
static void InitCommonData(void)
{
    ...
    ifgen_le_temp_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_temp_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
{
    return ifgen_le_temp_GetSensorName(
        GetCurrentSessionRef(),
        sensor,
        sensorName,
        sensorNameSize
...
{
    return ifgen_le_temp_AddThresholdEventHandler(
        GetCurrentSessionRef(),
        ThresholdEventHandlerPtr,
        contextPtr
    );
}
...
{
     ifgen_le_temp_RemoveThresholdEventHandler(
        GetCurrentSessionRef(),
        handlerRef
    );
}
...
{
    return ifgen_le_temp_GetTemperature(
        GetCurrentSessionRef(),
        sensor,
        temperaturePtr
    );
...
{
    return ifgen_le_temp_SetThreshold(
        GetCurrentSessionRef(),
        sensor,
        threshold,
        temperature
...
{
    return ifgen_le_temp_GetThreshold(
        GetCurrentSessionRef(),
        sensor,
        threshold,
        temperaturePtr
...
{
    return ifgen_le_temp_StartMonitoring(
        GetCurrentSessionRef()
    );
}
