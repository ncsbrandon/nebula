 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_appInfo_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_appInfo_ClientHandlers,
    LE_MEM_BLOCKS(le_appInfo_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_appInfo_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_appInfo_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_appInfo_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    if (!le_appInfo_UnpackState( &_msgBufPtr, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_appInfo_UnpackProcState( &_msgBufPtr, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (appName)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (appNameSize-1) ));
    }
