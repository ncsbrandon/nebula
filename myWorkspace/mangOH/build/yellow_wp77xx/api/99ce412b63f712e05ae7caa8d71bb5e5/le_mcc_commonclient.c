 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_mcc_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_mcc_ClientHandlers,
    LE_MEM_BLOCKS(le_mcc_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_mcc_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_mcc_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_mcc_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    if (telPtr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (telPtrSize-1) ));
    }
...
static void _Handle_ifgen_le_mcc_AddCallEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_mcc_Event_t event = (le_mcc_Event_t) 0;

    // Pull out additional data from the client data pointer
    le_mcc_CallEventHandlerFunc_t _handlerRef_ifgen_le_mcc_AddCallEventHandler =
        (le_mcc_CallEventHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!le_mcc_UnpackEvent( &_msgBufPtr,
                                               &event ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_le_mcc_AddCallEventHandler != NULL )
    {
        _handlerRef_ifgen_le_mcc_AddCallEventHandler(callRef, event, contextPtr );
    }
    else
    {
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_mcc_CallEventHandlerRef_t)
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_mcc_AddCallEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_mcc_AddCallEventHandler, msgRef, clientDataPtr);
            break;
