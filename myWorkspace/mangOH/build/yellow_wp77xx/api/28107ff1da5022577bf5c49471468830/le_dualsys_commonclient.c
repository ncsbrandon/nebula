 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_dualsys_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_dualsys_ClientHandlers,
    LE_MEM_BLOCKS(le_dualsys_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_dualsys_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_dualsys_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_dualsys_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    _msgPtr->id = _MSGID_le_dualsys_DisableSyncBeforeUpdate;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_dualsys_SetSystem;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_dualsys_GetCurrentSystem;
    _msgBufPtr = _msgPtr->buffer;

    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(systemMaskPtr)) << 0);
    ...
    // Unpack any "out" parameters
    if (systemMaskPtr &&
        (!le_dualsys_UnpackSystem( &_msgBufPtr,
                                               systemMaskPtr )))
    {
        goto error_unpack;
    }
