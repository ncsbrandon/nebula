...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_dualsys_System_t systemMask = (le_dualsys_System_t) 0;
    ...
    // Unpack the input parameters from the message
    if (!le_dualsys_UnpackSystem( &_msgBufPtr,
                                               &systemMask ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_dualsys_System_t systemMaskBuffer = (le_dualsys_System_t) 0;
    ...
    if (systemMaskPtr)
    {
        LE_ASSERT(le_dualsys_PackSystem( &_msgBufPtr,
                                                      *systemMaskPtr ));
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_dualsys_DisableSyncBeforeUpdate :
            Handle_le_dualsys_DisableSyncBeforeUpdate(msgRef);
            break;
        case _MSGID_le_dualsys_SetSystem :
            Handle_le_dualsys_SetSystem(msgRef);
            break;
        case _MSGID_le_dualsys_GetCurrentSystem :
            Handle_le_dualsys_GetCurrentSystem(msgRef);
            break;
