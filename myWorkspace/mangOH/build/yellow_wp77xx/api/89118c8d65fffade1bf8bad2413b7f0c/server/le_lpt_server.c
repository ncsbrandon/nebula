 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_lpt_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_lpt_ServerHandlers,
    LE_MEM_BLOCKS(le_lpt_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_lpt_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_lpt_ServerHandlers,
                        LE_MEM_BLOCKS(le_lpt_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    // Define storage for output parameters
    uint8_t pagingTimeWindowBuffer = 0;
...
)
{
    ...
    // Unpack the input parameters from the message
    le_lpt_EDrxParamsChangeHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_lpt_SetEDrxState :
            Handle_le_lpt_SetEDrxState(msgRef);
            break;
        case _MSGID_le_lpt_SetRequestedEDrxValue :
            Handle_le_lpt_SetRequestedEDrxValue(msgRef);
            break;
        case _MSGID_le_lpt_GetRequestedEDrxValue :
            Handle_le_lpt_GetRequestedEDrxValue(msgRef);
            break;
        case _MSGID_le_lpt_GetNetworkProvidedEDrxValue :
            Handle_le_lpt_GetNetworkProvidedEDrxValue(msgRef);
            break;
        case _MSGID_le_lpt_GetNetworkProvidedPagingTimeWindow :
            Handle_le_lpt_GetNetworkProvidedPagingTimeWindow(msgRef);
            break;
        case _MSGID_le_lpt_AddEDrxParamsChangeHandler :
            Handle_le_lpt_AddEDrxParamsChangeHandler(msgRef);
            break;
        case _MSGID_le_lpt_RemoveEDrxParamsChangeHandler :
            Handle_le_lpt_RemoveEDrxParamsChangeHandler(msgRef);
            break;
