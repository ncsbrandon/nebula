...
typedef struct
{
    ...
    le_port_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_port_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_port_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_port_HasLocalBinding())
    {
        return NULL;
    }
    else
    {
        ...
        LE_FATAL_IF(clientThreadPtr==NULL,
                    "le_port_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    ifgen_le_port_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_port_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
)
{
    ...
    LE_FATAL("Component for le_port disconnected\n");
}
...
{
    return ifgen_le_port_SetDataMode(
        GetCurrentSessionRef(),
        devRef,
        fdPtr
    );
...
{
    return ifgen_le_port_SetCommandMode(
        GetCurrentSessionRef(),
        devRef,
        atServerDevRefPtr
    );
...
{
    return ifgen_le_port_GetPortReference(
        GetCurrentSessionRef(),
        atServerDevRef,
        devRefPtr
    );
