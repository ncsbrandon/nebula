 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_mrc_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_mrc_ServerHandlers,
    LE_MEM_BLOCKS(le_mrc_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_mrc_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_mrc_ServerHandlers,
                        LE_MEM_BLOCKS(le_mrc_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    LE_ASSERT(le_mrc_PackNetRegState( &_msgBufPtr,
                                                  state ));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_NetRegStateEventHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    LE_ASSERT(le_mrc_PackRat( &_msgBufPtr,
                                                  rat ));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_RatChangeHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_PacketSwitchedChangeHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_Rat_t rat = (le_mrc_Rat_t) 0;
    int32_t lowerRangeThreshold = 0;
    int32_t upperRangeThreshold = 0;
    void *contextPtr = NULL;
    _ServerData_t* serverDataPtr = NULL;
    ...
    // Unpack the input parameters from the message
    if (!le_mrc_UnpackRat( &_msgBufPtr,
                                               &rat ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_SignalStrengthChangeHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
...
)
{
    ...
    // Define storage for output parameters
    le_mrc_NetRegState_t stateBuffer = (le_mrc_NetRegState_t) 0;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_RatBitMask_t ratMask = (le_mrc_RatBitMask_t) 0;
    ...
    // Unpack the input parameters from the message
    if (!le_mrc_UnpackRatBitMask( &_msgBufPtr,
                                               &ratMask ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_mrc_RatBitMask_t ratMaskPtrBuffer = (le_mrc_RatBitMask_t) 0;
    ...
    if (ratMaskPtrPtr)
    {
        LE_ASSERT(le_mrc_PackRatBitMask( &_msgBufPtr,
                                                      *ratMaskPtrPtr ));
    }
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_BandBitMask_t bandMask = (le_mrc_BandBitMask_t) 0;
    ...
    // Unpack the input parameters from the message
    if (!le_mrc_UnpackBandBitMask( &_msgBufPtr,
                                               &bandMask ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_mrc_BandBitMask_t bandMaskPtrBuffer = (le_mrc_BandBitMask_t) 0;
    ...
    if (bandMaskPtrPtr)
    {
        LE_ASSERT(le_mrc_PackBandBitMask( &_msgBufPtr,
                                                      *bandMaskPtrPtr ));
    }
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_LteBandBitMask_t bandMask = (le_mrc_LteBandBitMask_t) 0;
    ...
    // Unpack the input parameters from the message
    if (!le_mrc_UnpackLteBandBitMask( &_msgBufPtr,
                                               &bandMask ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_mrc_LteBandBitMask_t bandMaskPtrBuffer = (le_mrc_LteBandBitMask_t) 0;
    ...
    if (bandMaskPtrPtr)
    {
        LE_ASSERT(le_mrc_PackLteBandBitMask( &_msgBufPtr,
                                                      *bandMaskPtrPtr ));
    }
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_TdScdmaBandBitMask_t bandMask = (le_mrc_TdScdmaBandBitMask_t) 0;
    ...
    // Unpack the input parameters from the message
    if (!le_mrc_UnpackTdScdmaBandBitMask( &_msgBufPtr,
                                               &bandMask ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_mrc_TdScdmaBandBitMask_t bandMaskPtrBuffer = (le_mrc_TdScdmaBandBitMask_t) 0;
    ...
    if (bandMaskPtrPtr)
    {
        LE_ASSERT(le_mrc_PackTdScdmaBandBitMask( &_msgBufPtr,
                                                      *bandMaskPtrPtr ));
    }
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_PreferredOperatorListRef_t preferredOperatorListRef = NULL;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_PreferredOperatorRef_t preferredOperatorRef = NULL;
    size_t mccPtrSize = 0;
    size_t mncPtrSize = 0;
    ...
    char mncPtrBuffer[4] = { 0 };
    char *mncPtr = mncPtrBuffer;
    le_mrc_RatBitMask_t ratMaskBuffer = (le_mrc_RatBitMask_t) 0;
...
static void Handle_le_mrc_PerformPciNetworkScan
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = le_mrc_PerformPciNetworkScan ( 
        ratMask );
...
static void AsyncResponse_le_mrc_PerformPciNetworkScanAsync
(
...
)
{
    ...
    if ( serverDataPtr->clientSessionRef == NULL )
    {
        LE_FATAL("Handler passed to le_mrc_PerformPciNetworkScanAsync() can't be called more than once");
    }
...
static void Handle_le_mrc_PerformPciNetworkScanAsync
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    le_mrc_PerformPciNetworkScanAsync ( 
        ratMask, AsyncResponse_le_mrc_PerformPciNetworkScanAsync, 
        contextPtr );
...
static void Handle_le_mrc_GetFirstPciScanInfo
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = le_mrc_GetFirstPciScanInfo ( 
        scanInformationListRef );
...
static void Handle_le_mrc_GetNextPciScanInfo
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_mrc_PciScanInformationRef_t _result;
    _result  = le_mrc_GetNextPciScanInfo ( 
        scanInformationListRef );
...
static void Handle_le_mrc_GetFirstPlmnInfo
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_PciScanInformationRef_t pciScanInformationRef = NULL;
    ...
    _result  = le_mrc_GetFirstPlmnInfo ( 
        pciScanInformationRef );
...
static void Handle_le_mrc_GetNextPlmnInfo
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_PciScanInformationRef_t plmnRef = NULL;
    ...
    // Call the function
    le_mrc_PlmnInformationRef_t _result;
    _result  = le_mrc_GetNextPlmnInfo ( 
        plmnRef );
...
static void Handle_le_mrc_GetPciScanCellId
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    uint16_t _result;
    _result  = le_mrc_GetPciScanCellId ( 
        pciScanInformationRef );
...
static void Handle_le_mrc_GetPciScanGlobalCellId
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    uint32_t _result;
    _result  = le_mrc_GetPciScanGlobalCellId ( 
        pciScanInformationRef );
...
static void Handle_le_mrc_GetPciScanMccMnc
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_PlmnInformationRef_t plmnRef = NULL;
    size_t mccPtrSize = 0;
    size_t mncPtrSize = 0;
    ...
    // Call the function
    le_result_t _result;
    _result  = le_mrc_GetPciScanMccMnc ( 
        plmnRef, 
        mccPtr, 
        mccPtrSize, 
        mncPtr, 
...
static void Handle_le_mrc_DeletePciNetworkScan
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_ScanInformationRef_t scanInformationRef = NULL;
    size_t mccPtrSize = 0;
    size_t mncPtrSize = 0;
...
)
{
    ...
    // Declare temporaries for input parameters
    le_mrc_ScanInformationRef_t scanInformationRef = NULL;
    size_t namePtrSize = 0;

    // Define storage for output parameters
    char namePtrBuffer[101] = { 0 };
    ...
    {
        LE_DEBUG("Adjusting namePtrSize from %" PRIuS " to 100", namePtrSize);
        namePtrSize = 100;
    }
...
)
{
    ...
    // Pack the result first
    LE_ASSERT(le_mrc_PackRat( &_msgBufPtr, _result ));
...
)
{
    ...
    // Define storage for output parameters
    char mccStrBuffer[4] = { 0 };
    char *mccStr = mccStrBuffer;
    char mncStrBuffer[4] = { 0 };
    ...
    {
        LE_DEBUG("Adjusting mccStrSize from %" PRIuS " to 3", mccStrSize);
        mccStrSize = 3;
    }
    ...
    {
        LE_DEBUG("Adjusting mncStrSize from %" PRIuS " to 3", mncStrSize);
        mncStrSize = 3;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_mrc_Rat_t ratBuffer = (le_mrc_Rat_t) 0;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_NeighborCellsRef_t ngbrCellsRef = NULL;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_CellInfoRef_t ngbrCellInfoRef = NULL;
...
)
{
    ...
    // Define storage for output parameters
    int32_t rsrqBuffer = 0;
    int32_t *rsrqPtr = &rsrqBuffer;
    int32_t rsrpBuffer = 0;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_mrc_MetricsRef_t MetricsRef = NULL;
...
)
{
    ...
    // Define storage for output parameters
    int32_t rssiBuffer = 0;
    int32_t *rssiPtr = &rssiBuffer;
    uint32_t berBuffer = 0;
...
)
{
    ...
    uint32_t blerBuffer = 0;
    uint32_t *blerPtr = &blerBuffer;
    int32_t ecioBuffer = 0;
    int32_t *ecioPtr = &ecioBuffer;
    int32_t rscpBuffer = 0;
    int32_t *rscpPtr = &rscpBuffer;
    int32_t sinrBuffer = 0;
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_NetworkRejectHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
...
static void AsyncResponse_le_mrc_AddNetRegRejectHandler
(
...
)
{
    ...
    LE_ASSERT(le_mrc_PackNetRegRejectInd( &_msgBufPtr,
                                                  networkRejectIndPtr ));
...
static void Handle_le_mrc_AddNetRegRejectHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = le_mrc_AddNetRegRejectHandler ( AsyncResponse_le_mrc_AddNetRegRejectHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->handlerRef = (le_event_HandlerRef_t)_result;
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)le_mrc_RemoveNetRegRejectHandler;
...
static void Handle_le_mrc_RemoveNetRegRejectHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_NetRegRejectHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (le_mrc_NetRegRejectHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);
...
)
{
    ...
    LE_ASSERT(le_mrc_PackJammingReport( &_msgBufPtr,
                                                  report ));
    LE_ASSERT(le_mrc_PackJammingStatus( &_msgBufPtr,
                                                  status ));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_mrc_JammingDetectionEventHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_mrc_AddNetRegStateEventHandler :
            Handle_le_mrc_AddNetRegStateEventHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemoveNetRegStateEventHandler :
            Handle_le_mrc_RemoveNetRegStateEventHandler(msgRef);
            break;
        case _MSGID_le_mrc_AddRatChangeHandler :
            Handle_le_mrc_AddRatChangeHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemoveRatChangeHandler :
            Handle_le_mrc_RemoveRatChangeHandler(msgRef);
            break;
        case _MSGID_le_mrc_AddPacketSwitchedChangeHandler :
            Handle_le_mrc_AddPacketSwitchedChangeHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemovePacketSwitchedChangeHandler :
            Handle_le_mrc_RemovePacketSwitchedChangeHandler(msgRef);
            break;
        case _MSGID_le_mrc_AddSignalStrengthChangeHandler :
            Handle_le_mrc_AddSignalStrengthChangeHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemoveSignalStrengthChangeHandler :
            Handle_le_mrc_RemoveSignalStrengthChangeHandler(msgRef);
            break;
        case _MSGID_le_mrc_SetSignalStrengthIndThresholds :
            Handle_le_mrc_SetSignalStrengthIndThresholds(msgRef);
            break;
        case _MSGID_le_mrc_SetSignalStrengthIndDelta :
            Handle_le_mrc_SetSignalStrengthIndDelta(msgRef);
            break;
        case _MSGID_le_mrc_SetAutomaticRegisterMode :
            Handle_le_mrc_SetAutomaticRegisterMode(msgRef);
            break;
        case _MSGID_le_mrc_SetManualRegisterMode :
            Handle_le_mrc_SetManualRegisterMode(msgRef);
            break;
        case _MSGID_le_mrc_SetManualRegisterModeAsync :
            Handle_le_mrc_SetManualRegisterModeAsync(msgRef);
            break;
        case _MSGID_le_mrc_GetRegisterMode :
            Handle_le_mrc_GetRegisterMode(msgRef);
            break;
        case _MSGID_le_mrc_GetPacketSwitchedState :
            Handle_le_mrc_GetPacketSwitchedState(msgRef);
            break;
        case _MSGID_le_mrc_GetPlatformSpecificRegistrationErrorCode :
            Handle_le_mrc_GetPlatformSpecificRegistrationErrorCode(msgRef);
            break;
        case _MSGID_le_mrc_SetRatPreferences :
            Handle_le_mrc_SetRatPreferences(msgRef);
            break;
        case _MSGID_le_mrc_GetRatPreferences :
            Handle_le_mrc_GetRatPreferences(msgRef);
            break;
        case _MSGID_le_mrc_SetBandPreferences :
            Handle_le_mrc_SetBandPreferences(msgRef);
            break;
        case _MSGID_le_mrc_GetBandPreferences :
            Handle_le_mrc_GetBandPreferences(msgRef);
            break;
        case _MSGID_le_mrc_SetLteBandPreferences :
            Handle_le_mrc_SetLteBandPreferences(msgRef);
            break;
        case _MSGID_le_mrc_GetLteBandPreferences :
            Handle_le_mrc_GetLteBandPreferences(msgRef);
            break;
        case _MSGID_le_mrc_SetTdScdmaBandPreferences :
            Handle_le_mrc_SetTdScdmaBandPreferences(msgRef);
            break;
        case _MSGID_le_mrc_GetTdScdmaBandPreferences :
            Handle_le_mrc_GetTdScdmaBandPreferences(msgRef);
            break;
        case _MSGID_le_mrc_AddPreferredOperator :
            Handle_le_mrc_AddPreferredOperator(msgRef);
            break;
        case _MSGID_le_mrc_RemovePreferredOperator :
            Handle_le_mrc_RemovePreferredOperator(msgRef);
            break;
        case _MSGID_le_mrc_GetPreferredOperatorsList :
            Handle_le_mrc_GetPreferredOperatorsList(msgRef);
            break;
        case _MSGID_le_mrc_GetFirstPreferredOperator :
            Handle_le_mrc_GetFirstPreferredOperator(msgRef);
            break;
        case _MSGID_le_mrc_GetNextPreferredOperator :
            Handle_le_mrc_GetNextPreferredOperator(msgRef);
            break;
        case _MSGID_le_mrc_DeletePreferredOperatorsList :
            Handle_le_mrc_DeletePreferredOperatorsList(msgRef);
            break;
        case _MSGID_le_mrc_GetPreferredOperatorDetails :
            Handle_le_mrc_GetPreferredOperatorDetails(msgRef);
            break;
        case _MSGID_le_mrc_GetNetRegState :
            Handle_le_mrc_GetNetRegState(msgRef);
            break;
        case _MSGID_le_mrc_GetSignalQual :
            Handle_le_mrc_GetSignalQual(msgRef);
            break;
        case _MSGID_le_mrc_SetRadioPower :
            Handle_le_mrc_SetRadioPower(msgRef);
            break;
        case _MSGID_le_mrc_GetRadioPower :
            Handle_le_mrc_GetRadioPower(msgRef);
            break;
        case _MSGID_le_mrc_PerformCellularNetworkScan :
            Handle_le_mrc_PerformCellularNetworkScan(msgRef);
            break;
        case _MSGID_le_mrc_PerformPciNetworkScan :
            Handle_le_mrc_PerformPciNetworkScan(msgRef);
            break;
        case _MSGID_le_mrc_PerformCellularNetworkScanAsync :
            Handle_le_mrc_PerformCellularNetworkScanAsync(msgRef);
            break;
        case _MSGID_le_mrc_PerformPciNetworkScanAsync :
            Handle_le_mrc_PerformPciNetworkScanAsync(msgRef);
            break;
        case _MSGID_le_mrc_GetFirstCellularNetworkScan :
            Handle_le_mrc_GetFirstCellularNetworkScan(msgRef);
            break;
        case _MSGID_le_mrc_GetNextCellularNetworkScan :
            Handle_le_mrc_GetNextCellularNetworkScan(msgRef);
            break;
        case _MSGID_le_mrc_GetFirstPciScanInfo :
            Handle_le_mrc_GetFirstPciScanInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetNextPciScanInfo :
            Handle_le_mrc_GetNextPciScanInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetFirstPlmnInfo :
            Handle_le_mrc_GetFirstPlmnInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetNextPlmnInfo :
            Handle_le_mrc_GetNextPlmnInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetPciScanCellId :
            Handle_le_mrc_GetPciScanCellId(msgRef);
            break;
        case _MSGID_le_mrc_GetPciScanGlobalCellId :
            Handle_le_mrc_GetPciScanGlobalCellId(msgRef);
            break;
        case _MSGID_le_mrc_GetPciScanMccMnc :
            Handle_le_mrc_GetPciScanMccMnc(msgRef);
            break;
        case _MSGID_le_mrc_DeleteCellularNetworkScan :
            Handle_le_mrc_DeleteCellularNetworkScan(msgRef);
            break;
        case _MSGID_le_mrc_DeletePciNetworkScan :
            Handle_le_mrc_DeletePciNetworkScan(msgRef);
            break;
        case _MSGID_le_mrc_GetCellularNetworkMccMnc :
            Handle_le_mrc_GetCellularNetworkMccMnc(msgRef);
            break;
        case _MSGID_le_mrc_GetCellularNetworkName :
            Handle_le_mrc_GetCellularNetworkName(msgRef);
            break;
        case _MSGID_le_mrc_GetCellularNetworkRat :
            Handle_le_mrc_GetCellularNetworkRat(msgRef);
            break;
        case _MSGID_le_mrc_IsCellularNetworkInUse :
            Handle_le_mrc_IsCellularNetworkInUse(msgRef);
            break;
        case _MSGID_le_mrc_IsCellularNetworkAvailable :
            Handle_le_mrc_IsCellularNetworkAvailable(msgRef);
            break;
        case _MSGID_le_mrc_IsCellularNetworkHome :
            Handle_le_mrc_IsCellularNetworkHome(msgRef);
            break;
        case _MSGID_le_mrc_IsCellularNetworkForbidden :
            Handle_le_mrc_IsCellularNetworkForbidden(msgRef);
            break;
        case _MSGID_le_mrc_GetCurrentNetworkName :
            Handle_le_mrc_GetCurrentNetworkName(msgRef);
            break;
        case _MSGID_le_mrc_GetCurrentNetworkMccMnc :
            Handle_le_mrc_GetCurrentNetworkMccMnc(msgRef);
            break;
        case _MSGID_le_mrc_GetRadioAccessTechInUse :
            Handle_le_mrc_GetRadioAccessTechInUse(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellsInfo :
            Handle_le_mrc_GetNeighborCellsInfo(msgRef);
            break;
        case _MSGID_le_mrc_DeleteNeighborCellsInfo :
            Handle_le_mrc_DeleteNeighborCellsInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetFirstNeighborCellInfo :
            Handle_le_mrc_GetFirstNeighborCellInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetNextNeighborCellInfo :
            Handle_le_mrc_GetNextNeighborCellInfo(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellId :
            Handle_le_mrc_GetNeighborCellId(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellLocAreaCode :
            Handle_le_mrc_GetNeighborCellLocAreaCode(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellRxLevel :
            Handle_le_mrc_GetNeighborCellRxLevel(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellRat :
            Handle_le_mrc_GetNeighborCellRat(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellUmtsEcIo :
            Handle_le_mrc_GetNeighborCellUmtsEcIo(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellLteIntraFreq :
            Handle_le_mrc_GetNeighborCellLteIntraFreq(msgRef);
            break;
        case _MSGID_le_mrc_GetNeighborCellLteInterFreq :
            Handle_le_mrc_GetNeighborCellLteInterFreq(msgRef);
            break;
        case _MSGID_le_mrc_MeasureSignalMetrics :
            Handle_le_mrc_MeasureSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_DeleteSignalMetrics :
            Handle_le_mrc_DeleteSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_GetRatOfSignalMetrics :
            Handle_le_mrc_GetRatOfSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_GetGsmSignalMetrics :
            Handle_le_mrc_GetGsmSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_GetUmtsSignalMetrics :
            Handle_le_mrc_GetUmtsSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_GetLteSignalMetrics :
            Handle_le_mrc_GetLteSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_GetCdmaSignalMetrics :
            Handle_le_mrc_GetCdmaSignalMetrics(msgRef);
            break;
        case _MSGID_le_mrc_GetServingCellId :
            Handle_le_mrc_GetServingCellId(msgRef);
            break;
        case _MSGID_le_mrc_GetServingCellLocAreaCode :
            Handle_le_mrc_GetServingCellLocAreaCode(msgRef);
            break;
        case _MSGID_le_mrc_GetServingCellLteTracAreaCode :
            Handle_le_mrc_GetServingCellLteTracAreaCode(msgRef);
            break;
        case _MSGID_le_mrc_GetBandCapabilities :
            Handle_le_mrc_GetBandCapabilities(msgRef);
            break;
        case _MSGID_le_mrc_GetLteBandCapabilities :
            Handle_le_mrc_GetLteBandCapabilities(msgRef);
            break;
        case _MSGID_le_mrc_GetTdScdmaBandCapabilities :
            Handle_le_mrc_GetTdScdmaBandCapabilities(msgRef);
            break;
        case _MSGID_le_mrc_AddNetworkRejectHandler :
            Handle_le_mrc_AddNetworkRejectHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemoveNetworkRejectHandler :
            Handle_le_mrc_RemoveNetworkRejectHandler(msgRef);
            break;
        case _MSGID_le_mrc_AddNetRegRejectHandler :
            Handle_le_mrc_AddNetRegRejectHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemoveNetRegRejectHandler :
            Handle_le_mrc_RemoveNetRegRejectHandler(msgRef);
            break;
        case _MSGID_le_mrc_AddJammingDetectionEventHandler :
            Handle_le_mrc_AddJammingDetectionEventHandler(msgRef);
            break;
        case _MSGID_le_mrc_RemoveJammingDetectionEventHandler :
            Handle_le_mrc_RemoveJammingDetectionEventHandler(msgRef);
            break;
        case _MSGID_le_mrc_StartJammingDetection :
            Handle_le_mrc_StartJammingDetection(msgRef);
            break;
        case _MSGID_le_mrc_StopJammingDetection :
            Handle_le_mrc_StopJammingDetection(msgRef);
            break;
        case _MSGID_le_mrc_SetSarBackoffState :
            Handle_le_mrc_SetSarBackoffState(msgRef);
            break;
        case _MSGID_le_mrc_GetSarBackoffState :
            Handle_le_mrc_GetSarBackoffState(msgRef);
            break;
