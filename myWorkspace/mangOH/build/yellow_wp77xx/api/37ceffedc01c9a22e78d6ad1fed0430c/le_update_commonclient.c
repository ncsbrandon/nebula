 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_update_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_update_ClientHandlers,
    LE_MEM_BLOCKS(le_update_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_update_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_update_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_update_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
static void _Handle_ifgen_le_update_AddProgressHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_update_State_t updateState = (le_update_State_t) 0;
    uint32_t percentDone = 0;

    // Pull out additional data from the client data pointer
    le_update_ProgressHandlerFunc_t _handlerRef_ifgen_le_update_AddProgressHandler =
        (le_update_ProgressHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!le_update_UnpackState( &_msgBufPtr,
                                               &updateState ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_le_update_AddProgressHandler != NULL )
    {
        _handlerRef_ifgen_le_update_AddProgressHandler(updateState, percentDone, contextPtr );
    }
    else
    {
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_update_ProgressHandlerRef_t)
...
)
{
    ...
    // Unpack the result first
    if (!le_update_UnpackErrorCode( &_msgBufPtr, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (hashStr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (hashStrSize-1) ));
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_update_AddProgressHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_update_AddProgressHandler, msgRef, clientDataPtr);
            break;
