 ...
 */
//--------------------------------------------------------------------------------------------------
#define HIGH_CLIENT_DATA_COUNT   4
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_avc_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_avc_ClientHandlers,
    LE_MEM_BLOCKS(le_avc_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_avc_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_avc_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_avc_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
static void _Handle_ifgen_le_avc_AddStatusEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_avc_Status_t updateStatus = (le_avc_Status_t) 0;
    int32_t totalNumBytes = 0;
    int32_t progress = 0;

    // Pull out additional data from the client data pointer
    le_avc_StatusHandlerFunc_t _handlerRef_ifgen_le_avc_AddStatusEventHandler =
        (le_avc_StatusHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!le_avc_UnpackStatus( &_msgBufPtr,
                                               &updateStatus ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_le_avc_AddStatusEventHandler != NULL )
    {
        _handlerRef_ifgen_le_avc_AddStatusEventHandler(updateStatus, totalNumBytes, progress, contextPtr );
    }
    else
    {
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_avc_StatusEventHandlerRef_t)
...
// This function parses the message buffer received from the server, and then calls the user
// registered handler, which is stored in a client data object.
static void _Handle_ifgen_le_avc_AddSessionRequestEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    // Declare temporaries for input parameters
    le_avc_SessionRequest_t request = (le_avc_SessionRequest_t) 0;

    // Pull out additional data from the client data pointer
    le_avc_SessionRequestHandlerFunc_t _handlerRef_ifgen_le_avc_AddSessionRequestEventHandler =
        (le_avc_SessionRequestHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;
    ...
    // Unpack the remaining parameters.
    if (!le_avc_UnpackSessionRequest( &_msgBufPtr,
                                               &request ))
    {
        goto error_unpack;
    }
    ...
    // Call the registered handler
    if ( _handlerRef_ifgen_le_avc_AddSessionRequestEventHandler != NULL )
    {
        _handlerRef_ifgen_le_avc_AddSessionRequestEventHandler(request, contextPtr );
    }
    else
    {
        LE_FATAL("Error in client data: no registered handler");
...
)
{
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (le_avc_SessionRequestEventHandlerRef_t)
         clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr,
                                     handlerRef ));
...
// This function parses the message buffer received from the server, and then calls the user
// registered handler, which is stored in a client data object.
static void _Handle_ifgen_le_avc_AddCommInfoHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    // Pull out additional data from the client data pointer
    le_avc_CommInfoHandlerFunc_t _handlerRef_ifgen_le_avc_AddCommInfoHandler =
        (le_avc_CommInfoHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;
    ...
    // Call the registered handler
    if ( _handlerRef_ifgen_le_avc_AddCommInfoHandler != NULL )
    {
        _handlerRef_ifgen_le_avc_AddCommInfoHandler(code, str, contextPtr );
    }
    else
    {
        LE_FATAL("Error in client data: no registered handler");
...
)
{
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (le_avc_CommInfoHandlerRef_t)
         clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr,
                                     handlerRef ));
...
)
{
    ...
    // Unpack any "out" parameters
    if (updateTypePtr &&
        (!le_avc_UnpackUpdateType( &_msgBufPtr,
                                               updateTypePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (updateName)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (updateNameSize-1) ));
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_avc_UnpackErrorCode( &_msgBufPtr, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_avc_UnpackSessionType( &_msgBufPtr, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (timerValuePtr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (*timerValueSizePtr) ));
    }
...
)
{
    ...
    if (apnName)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (apnNameSize-1) ));
    }
    if (userName)
    {
    ...
    if (userPwd)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (userPwdSize-1) ));
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_avc_UnpackCredentialStatus( &_msgBufPtr, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_avc_PackUserAgreement( &_msgBufPtr,
                                                  updateStatus ));
...
)
{
    ...
    if (data)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (dataSize-1) ));
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_avc_AddStatusEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_avc_AddStatusEventHandler, msgRef, clientDataPtr);
            break;
        case _MSGID_le_avc_AddSessionRequestEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_avc_AddSessionRequestEventHandler, msgRef, clientDataPtr);
            break;
        case _MSGID_le_avc_AddCommInfoHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_avc_AddCommInfoHandler, msgRef, clientDataPtr);
            break;
