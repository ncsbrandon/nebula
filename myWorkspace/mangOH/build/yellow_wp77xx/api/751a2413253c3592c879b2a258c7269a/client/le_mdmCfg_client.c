...
#include "le_mdmCfg_interface.h"
#include "le_mdmCfg_messages.h"
#include "le_mdmCfg_service.h"
...
typedef struct
{
    ...
    le_mdmCfg_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_mdmCfg_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_mdmCfg_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_mdmCfg_HasLocalBinding())
    {
        return NULL;
    }
    else
    {
        ...
        LE_FATAL_IF(clientThreadPtr==NULL,
                    "le_mdmCfg_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    ifgen_le_mdmCfg_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_mdmCfg_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
)
{
    ...
    LE_FATAL("Component for le_mdmCfg disconnected\n");
}
...
{
    return ifgen_le_mdmCfg_StoreCurrentConfiguration(
...
{
    return ifgen_le_mdmCfg_RestoreSavedConfiguration(
        GetCurrentSessionRef()
    );
}
