...
#include "le_bootReason_service.h"
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_bootReason_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_bootReason_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_bootReason_HasLocalBinding())
    {
        return NULL;
    }
    else
...
static void InitCommonData(void)
{
    ...
    ifgen_le_bootReason_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_bootReason_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
{
    return ifgen_le_bootReason_WasTimer(
...
{
    return ifgen_le_bootReason_WasGpio(
        GetCurrentSessionRef(),
        gpioNum
    );
}
...
{
    return ifgen_le_bootReason_WasAdc(
        GetCurrentSessionRef(),
        adcNum
    );
}
