...
#include "mangOH_bme680_server.h"
#include "mangOH_bme680_messages.h"
#include "mangOH_bme680_service.h"
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(mangOH_bme680_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(mangOH_bme680_ServerHandlers,
    LE_MEM_BLOCKS(mangOH_bme680_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(mangOH_bme680_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(mangOH_bme680_ServerHandlers,
                        LE_MEM_BLOCKS(mangOH_bme680_ServerData, HIGH_SERVER_DATA_COUNT));
...
static void Handle_mangOH_bme680_Configure
(
...
)
{
    ...
    mangOH_bme680_SamplingRate_t samplingRate = (mangOH_bme680_SamplingRate_t) 0;
    bool enableIaq = false;
    bool enableCo2Equivalent = false;
    bool enableBreathVoc = false;
    bool enablePressure = false;
    bool enableTemperature = false;
    bool enableHumidity = false;
    ...
    if (!mangOH_bme680_UnpackSamplingRate( &_msgBufPtr,
                                               &samplingRate ))
    {
        goto error_unpack;
    }
    ...
    // Call the function
    le_result_t _result;
    _result  = mangOH_bme680_Configure ( 
        samplingRate, 
        enableIaq, 
        enableCo2Equivalent, 
        enableBreathVoc, 
...
static void Handle_mangOH_bme680_SetAmbientTemperature
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    mangOH_bme680_SetAmbientTemperature ( 
        degC );
...
static void AsyncResponse_mangOH_bme680_AddSensorReadingHandler
(
...
)
{
    ...
    _msgPtr->id = _MSGID_mangOH_bme680_AddSensorReadingHandler;
    ...
    LE_ASSERT(mangOH_bme680_PackReading( &_msgBufPtr,
                                                  readingPtr ));
...
static void Handle_mangOH_bme680_AddSensorReadingHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    mangOH_bme680_SensorReadingHandlerRef_t _result;
    _result  = mangOH_bme680_AddSensorReadingHandler ( AsyncResponse_mangOH_bme680_AddSensorReadingHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)mangOH_bme680_RemoveSensorReadingHandler;
...
static void Handle_mangOH_bme680_RemoveSensorReadingHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Unpack the input parameters from the message
    mangOH_bme680_SensorReadingHandlerRef_t handlerRef = NULL;
    ...
    _UNLOCK
    handlerRef = (mangOH_bme680_SensorReadingHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);

    // Call the function
    mangOH_bme680_RemoveSensorReadingHandler ( 
        handlerRef );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_mangOH_bme680_Configure :
            Handle_mangOH_bme680_Configure(msgRef);
            break;
        case _MSGID_mangOH_bme680_SetAmbientTemperature :
            Handle_mangOH_bme680_SetAmbientTemperature(msgRef);
            break;
        case _MSGID_mangOH_bme680_AddSensorReadingHandler :
            Handle_mangOH_bme680_AddSensorReadingHandler(msgRef);
            break;
        case _MSGID_mangOH_bme680_RemoveSensorReadingHandler :
            Handle_mangOH_bme680_RemoveSensorReadingHandler(msgRef);
            break;
