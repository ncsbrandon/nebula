...
#include "mangOH_bme680_interface.h"
...
typedef struct
{
    ...
    mangOH_bme680_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(mangOH_bme680_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_mangOH_bme680_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_mangOH_bme680_HasLocalBinding())
    {
        return NULL;
    }
    else
    {
        ...
        LE_FATAL_IF(clientThreadPtr==NULL,
                    "mangOH_bme680_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    ifgen_mangOH_bme680_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(mangOH_bme680_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
)
{
    ...
    LE_FATAL("Component for mangOH_bme680 disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void mangOH_bme680_SetServerDisconnectHandler
(
...
{
    return ifgen_mangOH_bme680_Configure(
...
{
     ifgen_mangOH_bme680_SetAmbientTemperature(
        GetCurrentSessionRef(),
        degC
    );
}
...
{
    return ifgen_mangOH_bme680_AddSensorReadingHandler(
        GetCurrentSessionRef(),
        handlerPtr,
        contextPtr
    );
...
{
     ifgen_mangOH_bme680_RemoveSensorReadingHandler(
        GetCurrentSessionRef(),
        handlerRef
    );
}
