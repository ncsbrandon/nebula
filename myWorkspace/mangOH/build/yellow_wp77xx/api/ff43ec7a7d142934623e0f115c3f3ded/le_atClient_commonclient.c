 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_atClient_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_atClient_ClientHandlers,
    LE_MEM_BLOCKS(le_atClient_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_atClient_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_atClient_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_atClient_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(intermediate, 520) > 520 )
    {
        LE_FATAL("strnlen(intermediate, 520) > 520");
    }
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(response, 520) > 520 )
    {
        LE_FATAL("strnlen(response, 520) > 520");
    }
...
)
{
    ...
    if (intermediateRsp)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (intermediateRspSize-1) ));
    }
...
)
{
    ...
    if (finalRsp)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (finalRspSize-1) ));
    }
...
)
{
    ...
    if ( strnlen(interResp, 520) > 520 )
    {
        LE_FATAL("strnlen(interResp, 520) > 520");
    }
    if ( strnlen(finalResp, 520) > 520 )
    {
        LE_FATAL("strnlen(finalResp, 520) > 520");
    }
...
static void _Handle_ifgen_le_atClient_AddUnsolicitedResponseHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    char unsolicitedRsp[257] = {0};

    // Pull out additional data from the client data pointer
    le_atClient_UnsolicitedResponseHandlerFunc_t _handlerRef_ifgen_le_atClient_AddUnsolicitedResponseHandler =
        (le_atClient_UnsolicitedResponseHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_ifgen_le_atClient_AddUnsolicitedResponseHandler != NULL )
    {
        _handlerRef_ifgen_le_atClient_AddUnsolicitedResponseHandler(unsolicitedRsp, contextPtr );
    }
    else
    {
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(unsolRsp, 520) > 520 )
    {
        LE_FATAL("strnlen(unsolRsp, 520) > 520");
    }
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_atClient_UnsolicitedResponseHandlerRef_t)
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_atClient_AddUnsolicitedResponseHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_atClient_AddUnsolicitedResponseHandler, msgRef, clientDataPtr);
            break;
