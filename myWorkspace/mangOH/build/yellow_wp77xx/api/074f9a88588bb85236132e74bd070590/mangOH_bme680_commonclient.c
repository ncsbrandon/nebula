 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(mangOH_bme680_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(mangOH_bme680_ClientHandlers,
    LE_MEM_BLOCKS(mangOH_bme680_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(mangOH_bme680_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(mangOH_bme680_ClientHandlers,
                                                  LE_MEM_BLOCKS(mangOH_bme680_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    _msgPtr->id = _MSGID_mangOH_bme680_Configure;
    _msgBufPtr = _msgPtr->buffer;

    // Pack a list of outputs requested by the client.

    // Pack the input parameters
    LE_ASSERT(mangOH_bme680_PackSamplingRate( &_msgBufPtr,
                                                  samplingRate ));
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangOH_bme680_SetAmbientTemperature;
    _msgBufPtr = _msgPtr->buffer;
...
static void _Handle_ifgen_mangOH_bme680_AddSensorReadingHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    mangOH_bme680_Reading_t reading = {0, {false, 0, 0}, {false, 0, 0}, {false, 0, 0}, {false, 0}, {false, 0}, {false, 0}};
    mangOH_bme680_Reading_t *readingPtr =
        &reading;

    // Pull out additional data from the client data pointer
    mangOH_bme680_SensorReadingHandlerFunc_t _handlerRef_ifgen_mangOH_bme680_AddSensorReadingHandler =
        (mangOH_bme680_SensorReadingHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!mangOH_bme680_UnpackReading( &_msgBufPtr,
                                               &reading ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_mangOH_bme680_AddSensorReadingHandler != NULL )
    {
        _handlerRef_ifgen_mangOH_bme680_AddSensorReadingHandler(readingPtr, contextPtr );
    }
    else
    {
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangOH_bme680_RemoveSensorReadingHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    _UNLOCK
    handlerRef = (mangOH_bme680_SensorReadingHandlerRef_t)
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_mangOH_bme680_AddSensorReadingHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_mangOH_bme680_AddSensorReadingHandler, msgRef, clientDataPtr);
            break;
