 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_pm_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_pm_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_pm_HasLocalBinding())
    {
        return NULL;
    }
    else
...
static void InitCommonData(void)
{
    ...
    ifgen_le_pm_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_pm_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
{
    return ifgen_le_pm_NewWakeupSource(
...
{
    return ifgen_le_pm_ForceRelaxAndDestroyAllWakeupSource(
        GetCurrentSessionRef()
    );
}
