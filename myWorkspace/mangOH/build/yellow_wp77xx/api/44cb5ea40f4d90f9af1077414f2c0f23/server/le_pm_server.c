...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_pm_WakeupSourceRef_t wsRef = NULL;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_pm_NewWakeupSource :
            Handle_le_pm_NewWakeupSource(msgRef);
            break;
        case _MSGID_le_pm_StayAwake :
            Handle_le_pm_StayAwake(msgRef);
            break;
        case _MSGID_le_pm_Relax :
            Handle_le_pm_Relax(msgRef);
            break;
        case _MSGID_le_pm_ForceRelaxAndDestroyAllWakeupSource :
            Handle_le_pm_ForceRelaxAndDestroyAllWakeupSource(msgRef);
            break;
