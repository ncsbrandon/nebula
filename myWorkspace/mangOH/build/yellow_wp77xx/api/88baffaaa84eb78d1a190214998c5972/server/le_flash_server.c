 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_flash_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_flash_ServerHandlers,
    LE_MEM_BLOCKS(le_flash_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_flash_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_flash_ServerHandlers,
                        LE_MEM_BLOCKS(le_flash_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_flash_BadImageDetectionHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    char partitionName[52] = {0};
    le_flash_OpenMode_t mode = (le_flash_OpenMode_t) 0;

    // Define storage for output parameters
    le_flash_PartitionRef_t partitionRefBuffer = NULL;
    ...
    if (!le_flash_UnpackOpenMode( &_msgBufPtr,
                                               &mode ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_flash_PartitionRef_t partitionRef = NULL;
    char volumeName[52] = {0};
    int32_t volumeSize = 0;
...
)
{
    ...
    // Define storage for output parameters
    uint8_t readDataBuffer[262144] = { 0 };
    ...
    {
        LE_DEBUG("Adjusting readDataSize from %" PRIuS " to 262144", readDataSize);
        readDataSize = 262144;
    }
...
)
{
    ...
    bool isForcedCreate = false;
    uint32_t volumeID = 0;
    le_flash_UbiVolumeType_t volumeType = (le_flash_UbiVolumeType_t) 0;
    char volumeName[52] = {0};
    int32_t volumeSize = 0;
    ...
    if (!le_flash_UnpackUbiVolumeType( &_msgBufPtr,
                                               &volumeType ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_flash_AddBadImageDetectionHandler :
            Handle_le_flash_AddBadImageDetectionHandler(msgRef);
            break;
        case _MSGID_le_flash_RemoveBadImageDetectionHandler :
            Handle_le_flash_RemoveBadImageDetectionHandler(msgRef);
            break;
        case _MSGID_le_flash_RequestAccess :
            Handle_le_flash_RequestAccess(msgRef);
            break;
        case _MSGID_le_flash_ReleaseAccess :
            Handle_le_flash_ReleaseAccess(msgRef);
            break;
        case _MSGID_le_flash_OpenMtd :
            Handle_le_flash_OpenMtd(msgRef);
            break;
        case _MSGID_le_flash_OpenUbi :
            Handle_le_flash_OpenUbi(msgRef);
            break;
        case _MSGID_le_flash_OpenUbiVolume :
            Handle_le_flash_OpenUbiVolume(msgRef);
            break;
        case _MSGID_le_flash_CloseUbiVolume :
            Handle_le_flash_CloseUbiVolume(msgRef);
            break;
        case _MSGID_le_flash_Close :
            Handle_le_flash_Close(msgRef);
            break;
        case _MSGID_le_flash_EraseBlock :
            Handle_le_flash_EraseBlock(msgRef);
            break;
        case _MSGID_le_flash_Read :
            Handle_le_flash_Read(msgRef);
            break;
        case _MSGID_le_flash_Write :
            Handle_le_flash_Write(msgRef);
            break;
        case _MSGID_le_flash_GetBlockInformation :
            Handle_le_flash_GetBlockInformation(msgRef);
            break;
        case _MSGID_le_flash_GetUbiVolumeInformation :
            Handle_le_flash_GetUbiVolumeInformation(msgRef);
            break;
        case _MSGID_le_flash_CreateUbi :
            Handle_le_flash_CreateUbi(msgRef);
            break;
        case _MSGID_le_flash_CreateUbiVolume :
            Handle_le_flash_CreateUbiVolume(msgRef);
            break;
        case _MSGID_le_flash_DeleteUbiVolume :
            Handle_le_flash_DeleteUbiVolume(msgRef);
            break;
