 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_cellnet_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_cellnet_ServerHandlers,
    LE_MEM_BLOCKS(le_cellnet_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_cellnet_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_cellnet_ServerHandlers,
                        LE_MEM_BLOCKS(le_cellnet_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    LE_ASSERT(le_cellnet_PackState( &_msgBufPtr,
                                                  state ));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_cellnet_StateEventHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_cellnet_RequestObjRef_t cellNetRef = NULL;
...
)
{
    ...
    // Declare temporaries for input parameters
    le_sim_Id_t simId = (le_sim_Id_t) 0;
    size_t pinCodeSize = 0;

    // Define storage for output parameters
    char pinCodeBuffer[9] = { 0 };
    ...
    {
        LE_DEBUG("Adjusting pinCodeSize from %" PRIuS " to 8", pinCodeSize);
        pinCodeSize = 8;
    }
...
)
{
    ...
    // Define storage for output parameters
    le_cellnet_State_t stateBuffer = (le_cellnet_State_t) 0;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_cellnet_AddStateEventHandler :
            Handle_le_cellnet_AddStateEventHandler(msgRef);
            break;
        case _MSGID_le_cellnet_RemoveStateEventHandler :
            Handle_le_cellnet_RemoveStateEventHandler(msgRef);
            break;
        case _MSGID_le_cellnet_Request :
            Handle_le_cellnet_Request(msgRef);
            break;
        case _MSGID_le_cellnet_Release :
            Handle_le_cellnet_Release(msgRef);
            break;
        case _MSGID_le_cellnet_SetSimPinCode :
            Handle_le_cellnet_SetSimPinCode(msgRef);
            break;
        case _MSGID_le_cellnet_GetSimPinCode :
            Handle_le_cellnet_GetSimPinCode(msgRef);
            break;
        case _MSGID_le_cellnet_GetNetworkState :
            Handle_le_cellnet_GetNetworkState(msgRef);
            break;
