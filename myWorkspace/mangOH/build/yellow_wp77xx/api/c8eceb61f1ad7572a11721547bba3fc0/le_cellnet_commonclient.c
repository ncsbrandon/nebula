 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_cellnet_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_cellnet_ClientHandlers,
    LE_MEM_BLOCKS(le_cellnet_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_cellnet_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_cellnet_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_cellnet_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
static void _Handle_ifgen_le_cellnet_AddStateEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_cellnet_State_t state = (le_cellnet_State_t) 0;

    // Pull out additional data from the client data pointer
    le_cellnet_StateHandlerFunc_t _handlerRef_ifgen_le_cellnet_AddStateEventHandler =
        (le_cellnet_StateHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!le_cellnet_UnpackState( &_msgBufPtr,
                                               &state ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_le_cellnet_AddStateEventHandler != NULL )
    {
        _handlerRef_ifgen_le_cellnet_AddStateEventHandler(state, contextPtr );
    }
    else
    {
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_cellnet_StateEventHandlerRef_t)
...
)
{
    ...
    if (pinCode)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, (pinCodeSize-1) ));
    }
...
)
{
    ...
    // Unpack any "out" parameters
    if (statePtr &&
        (!le_cellnet_UnpackState( &_msgBufPtr,
                                               statePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_cellnet_AddStateEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_cellnet_AddStateEventHandler, msgRef, clientDataPtr);
            break;
