 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_coap_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_coap_ClientHandlers,
    LE_MEM_BLOCKS(le_coap_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_coap_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_coap_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_coap_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    if ( (NULL == tokenPtr) &&
         (0 != tokenSize) )
    {
        LE_FATAL("If tokenPtr is NULL "
                 "tokenSize must be zero");
    }
    if ( tokenSize > 9 )
    {
        LE_FATAL("tokenSize > 9");
    }
    if ( (NULL == payloadPtr) &&
         (0 != payloadSize) )
    {
        LE_FATAL("If payloadPtr is NULL "
                 "payloadSize must be zero");
    }
    if ( payloadSize > 1025 )
    {
        LE_FATAL("payloadSize > 1025");
    }
    ...
    _msgPtr->id = _MSGID_le_coap_SendResponse;
...
static void _Handle_ifgen_le_coap_AddMessageEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_coap_Code_t code = (le_coap_Code_t) 0;
    le_coap_StreamStatus_t stream = (le_coap_StreamStatus_t) 0;
    ...
    le_coap_MessageHandlerFunc_t _handlerRef_ifgen_le_coap_AddMessageEventHandler =
        (le_coap_MessageHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_ifgen_le_coap_AddMessageEventHandler != NULL )
    {
        _handlerRef_ifgen_le_coap_AddMessageEventHandler(code, stream, messageId, contentType, uri, tokenPtr, tokenSize, payloadPtr, payloadSize, blockSize, contextPtr );
    }
    else
    {
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_coap_RemoveMessageEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    _UNLOCK
    handlerRef = (le_coap_MessageEventHandlerRef_t)
...
// This function parses the message buffer received from the server, and then calls the user
// registered handler, which is stored in a client data object.
static void _Handle_ifgen_le_coap_AddPushEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    // Declare temporaries for input parameters
    le_coap_PushStatus_t status = (le_coap_PushStatus_t) 0;
    size_t tokenSize = 0;
    uint8_t
        tokenPtr[9]
            = { 0 };
    ...
    // Pull out additional data from the client data pointer
    le_coap_PushHandlerFunc_t _handlerRef_ifgen_le_coap_AddPushEventHandler =
        (le_coap_PushHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;
    ...
    // Unpack the remaining parameters.
    if (!le_coap_UnpackPushStatus( &_msgBufPtr,
                                               &status ))
    {
        goto error_unpack;
    }
    ...
    // Call the registered handler
    if ( _handlerRef_ifgen_le_coap_AddPushEventHandler != NULL )
    {
        _handlerRef_ifgen_le_coap_AddPushEventHandler(status, tokenPtr, tokenSize, contextPtr );
    }
    else
    {
        LE_FATAL("Error in client data: no registered handler");
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_coap_RemovePushEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (le_coap_PushEventHandlerRef_t)
         clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr,
                                     handlerRef ));
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(uri, 257) > 257 )
    {
        LE_FATAL("strnlen(uri, 257) > 257");
    }
    if ( (NULL == tokenPtr) &&
         (0 != tokenSize) )
    {
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_coap_Push;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_coap_AddMessageEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_coap_AddMessageEventHandler, msgRef, clientDataPtr);
            break;
        case _MSGID_le_coap_AddPushEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_coap_AddPushEventHandler, msgRef, clientDataPtr);
            break;
