 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_voicecall_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_voicecall_ClientHandlers,
    LE_MEM_BLOCKS(le_voicecall_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_voicecall_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_voicecall_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_voicecall_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
static void _Handle_ifgen_le_voicecall_AddStateHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_voicecall_CallRef_t reference = NULL;
    char identifier[18] = {0};
    le_voicecall_Event_t event = (le_voicecall_Event_t) 0;

    // Pull out additional data from the client data pointer
    le_voicecall_StateHandlerFunc_t _handlerRef_ifgen_le_voicecall_AddStateHandler =
        (le_voicecall_StateHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!le_voicecall_UnpackEvent( &_msgBufPtr,
                                               &event ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_le_voicecall_AddStateHandler != NULL )
    {
        _handlerRef_ifgen_le_voicecall_AddStateHandler(reference, identifier, event, contextPtr );
    }
    else
    {
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_RemoveStateHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    _UNLOCK
    handlerRef = (le_voicecall_StateHandlerRef_t)
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(DestinationID, 17) > 17 )
    {
        LE_FATAL("strnlen(DestinationID, 17) > 17");
    }
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_Start;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_End;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_Delete;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_Answer;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_GetTerminationReason;
    _msgBufPtr = _msgPtr->buffer;

    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(reasonPtr)) << 0);
    ...
    // Unpack any "out" parameters
    if (reasonPtr &&
        (!le_voicecall_UnpackTerminationReason( &_msgBufPtr,
                                               reasonPtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_GetTxAudioStream;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(_ifgen_sessionRef);
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_voicecall_GetRxAudioStream;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_voicecall_AddStateHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_voicecall_AddStateHandler, msgRef, clientDataPtr);
            break;
