...
#include "le_voicecall_service.h"
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_voicecall_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_voicecall_ServerHandlers,
    LE_MEM_BLOCKS(le_voicecall_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_voicecall_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_voicecall_ServerHandlers,
                        LE_MEM_BLOCKS(le_voicecall_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    LE_ASSERT(le_pack_PackString( &_msgBufPtr,
                                  identifier, 17 ));
    LE_ASSERT(le_voicecall_PackEvent( &_msgBufPtr,
                                                  event ));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_voicecall_StateHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    // Define storage for output parameters
    le_voicecall_TerminationReason_t reasonBuffer = (le_voicecall_TerminationReason_t) 0;
    ...
    if (reasonPtr)
    {
        LE_ASSERT(le_voicecall_PackTerminationReason( &_msgBufPtr,
                                                      *reasonPtr ));
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_voicecall_AddStateHandler :
            Handle_le_voicecall_AddStateHandler(msgRef);
            break;
        case _MSGID_le_voicecall_RemoveStateHandler :
            Handle_le_voicecall_RemoveStateHandler(msgRef);
            break;
        case _MSGID_le_voicecall_Start :
            Handle_le_voicecall_Start(msgRef);
            break;
        case _MSGID_le_voicecall_End :
            Handle_le_voicecall_End(msgRef);
            break;
        case _MSGID_le_voicecall_Delete :
            Handle_le_voicecall_Delete(msgRef);
            break;
        case _MSGID_le_voicecall_Answer :
            Handle_le_voicecall_Answer(msgRef);
            break;
        case _MSGID_le_voicecall_GetTerminationReason :
            Handle_le_voicecall_GetTerminationReason(msgRef);
            break;
        case _MSGID_le_voicecall_GetTxAudioStream :
            Handle_le_voicecall_GetTxAudioStream(msgRef);
            break;
        case _MSGID_le_voicecall_GetRxAudioStream :
            Handle_le_voicecall_GetRxAudioStream(msgRef);
            break;
