 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_ips_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_ips_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_ips_HasLocalBinding())
    {
        return NULL;
    }
    else
...
static void InitCommonData(void)
{
    ...
    ifgen_le_ips_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(le_ips_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
{
    return ifgen_le_ips_AddThresholdEventHandler(
...
{
     ifgen_le_ips_RemoveThresholdEventHandler(
        GetCurrentSessionRef(),
        handlerRef
    );
}
...
{
    return ifgen_le_ips_GetInputVoltage(
        GetCurrentSessionRef(),
        inputVoltagePtr
    );
}
...
{
    return ifgen_le_ips_SetVoltageThresholds(
        GetCurrentSessionRef(),
        criticalVolt,
        warningVolt,
        normalVolt,
...
{
    return ifgen_le_ips_GetVoltageThresholds(
        GetCurrentSessionRef(),
        criticalVoltPtr,
        warningVoltPtr,
        normalVoltPtr,
...
{
    return ifgen_le_ips_GetPowerSource(
        GetCurrentSessionRef(),
        powerSourcePtr
    );
}
...
{
    return ifgen_le_ips_GetBatteryLevel(
        GetCurrentSessionRef(),
        batteryLevelPtr
    );
}
...
{
    return ifgen_le_ips_SetBatteryLevel(
        GetCurrentSessionRef(),
        batteryLevel
    );
}
