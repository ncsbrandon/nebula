 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_ips_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_ips_ClientHandlers,
    LE_MEM_BLOCKS(le_ips_ClientData, HIGH_CLIENT_DATA_COUNT));
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(le_ips_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(le_ips_ClientHandlers,
                                                  LE_MEM_BLOCKS(le_ips_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
static void _Handle_ifgen_le_ips_AddThresholdEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_ips_ThresholdStatus_t event = (le_ips_ThresholdStatus_t) 0;

    // Pull out additional data from the client data pointer
    le_ips_ThresholdEventHandlerFunc_t _handlerRef_ifgen_le_ips_AddThresholdEventHandler =
        (le_ips_ThresholdEventHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if (!le_ips_UnpackThresholdStatus( &_msgBufPtr,
                                               &event ))
    {
        goto error_unpack;
    }
    ...
    if ( _handlerRef_ifgen_le_ips_AddThresholdEventHandler != NULL )
    {
        _handlerRef_ifgen_le_ips_AddThresholdEventHandler(event, contextPtr );
    }
    else
    {
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_ips_ThresholdEventHandlerRef_t)
...
)
{
    ...
    // Unpack any "out" parameters
    if (powerSourcePtr &&
        (!le_ips_UnpackPowerSource( &_msgBufPtr,
                                               powerSourcePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_ips_AddThresholdEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_le_ips_AddThresholdEventHandler, msgRef, clientDataPtr);
            break;
