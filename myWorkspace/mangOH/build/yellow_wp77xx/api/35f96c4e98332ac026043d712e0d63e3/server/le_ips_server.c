 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_ips_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_ips_ServerHandlers,
    LE_MEM_BLOCKS(le_ips_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_ips_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_ips_ServerHandlers,
                        LE_MEM_BLOCKS(le_ips_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    LE_ASSERT(le_ips_PackThresholdStatus( &_msgBufPtr,
                                                  event ));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_ips_ThresholdEventHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    uint16_t criticalVolt = 0;
    uint16_t warningVolt = 0;
    uint16_t normalVolt = 0;
    uint16_t hiCriticalVolt = 0;
...
)
{
    ...
    // Define storage for output parameters
    le_ips_PowerSource_t powerSourceBuffer = (le_ips_PowerSource_t) 0;
    ...
    if (powerSourcePtr)
    {
        LE_ASSERT(le_ips_PackPowerSource( &_msgBufPtr,
                                                      *powerSourcePtr ));
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_ips_AddThresholdEventHandler :
            Handle_le_ips_AddThresholdEventHandler(msgRef);
            break;
        case _MSGID_le_ips_RemoveThresholdEventHandler :
            Handle_le_ips_RemoveThresholdEventHandler(msgRef);
            break;
        case _MSGID_le_ips_GetInputVoltage :
            Handle_le_ips_GetInputVoltage(msgRef);
            break;
        case _MSGID_le_ips_SetVoltageThresholds :
            Handle_le_ips_SetVoltageThresholds(msgRef);
            break;
        case _MSGID_le_ips_GetVoltageThresholds :
            Handle_le_ips_GetVoltageThresholds(msgRef);
            break;
        case _MSGID_le_ips_GetPowerSource :
            Handle_le_ips_GetPowerSource(msgRef);
            break;
        case _MSGID_le_ips_GetBatteryLevel :
            Handle_le_ips_GetBatteryLevel(msgRef);
            break;
        case _MSGID_le_ips_SetBatteryLevel :
            Handle_le_ips_SetBatteryLevel(msgRef);
            break;
