...
#include "le_tpf_server.h"
#include "le_tpf_messages.h"
#include "le_tpf_service.h"
...
static void Handle_le_tpf_SetPackageUri
(
...
static void Handle_le_tpf_GetPackageUri
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    size_t packageUriSize = 0;

    // Define storage for output parameters
    char packageUriBuffer[256] = { 0 };
    char *packageUri = packageUriBuffer;
    ...
    if ( (packageUriSize > 255) &&
         (packageUriSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting packageUriSize from %" PRIuS " to 255", packageUriSize);
        packageUriSize = 255;
    }
    if (packageUriSize >= UINT32_MAX)
    {
        packageUriSize = UINT32_MAX;
    }
    else
...
static void Handle_le_tpf_Start
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_tpf_IsTpfStarted
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_tpf_SetPackageUri :
            Handle_le_tpf_SetPackageUri(msgRef);
            break;
        case _MSGID_le_tpf_GetPackageUri :
            Handle_le_tpf_GetPackageUri(msgRef);
            break;
        case _MSGID_le_tpf_Start :
            Handle_le_tpf_Start(msgRef);
            break;
        case _MSGID_le_tpf_IsTpfStarted :
            Handle_le_tpf_IsTpfStarted(msgRef);
            break;
