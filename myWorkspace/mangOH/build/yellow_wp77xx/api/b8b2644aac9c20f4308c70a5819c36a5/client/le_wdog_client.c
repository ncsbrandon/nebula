 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_wdog_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_le_wdog_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
{
    if (ifgen_le_wdog_HasLocalBinding())
    {
        return NULL;
    }
    else
...
static void InitCommonData(void)
{
    ...
    _ClientThreadDataPool = le_mem_InitStaticPool(le_wdog_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
{
    return ifgen_le_wdog_GetWatchdogTimeout(
        GetCurrentSessionRef(),
        millisecondsPtr
    );
}
...
{
    return ifgen_le_wdog_GetMaxWatchdogTimeout(
        GetCurrentSessionRef(),
        millisecondsPtr
    );
}
