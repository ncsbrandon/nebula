 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_rsim_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_rsim_ServerHandlers,
    LE_MEM_BLOCKS(le_rsim_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_rsim_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_rsim_ServerHandlers,
                        LE_MEM_BLOCKS(le_rsim_ServerData, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    // Unpack the input parameters from the message
    le_rsim_MessageHandlerRef_t handlerRef = NULL;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_rsim_AddMessageHandler :
            Handle_le_rsim_AddMessageHandler(msgRef);
            break;
        case _MSGID_le_rsim_RemoveMessageHandler :
            Handle_le_rsim_RemoveMessageHandler(msgRef);
            break;
        case _MSGID_le_rsim_SendMessage :
            Handle_le_rsim_SendMessage(msgRef);
            break;
