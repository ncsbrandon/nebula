 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(le_dcs_ServerData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(le_dcs_ServerHandlers,
    LE_MEM_BLOCKS(le_dcs_ServerCmd, HIGH_SERVER_DATA_COUNT));
...
)
{
    ...
    _ServerDataPool = le_mem_InitStaticPool(le_dcs_ServerData,
                                            HIGH_SERVER_DATA_COUNT,
                                            sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    _HandlerRefMap = le_ref_InitStaticMap(le_dcs_ServerHandlers,
                        LE_MEM_BLOCKS(le_dcs_ServerData, HIGH_SERVER_DATA_COUNT));
...
static void AsyncResponse_le_dcs_AddEventHandler
(
...
)
{
    ...
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr,
                                                  channelRef ));
    LE_ASSERT(le_dcs_PackEvent( &_msgBufPtr,
                                                  event ));
...
static void Handle_le_dcs_AddEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = le_dcs_AddEventHandler ( 
        channelRef, AsyncResponse_le_dcs_AddEventHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)le_dcs_RemoveEventHandler;
...
static void Handle_le_dcs_RemoveEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Unpack the input parameters from the message
    le_dcs_EventHandlerRef_t handlerRef = NULL;
    if (!le_pack_UnpackReference( &_msgBufPtr,
                                  &handlerRef ))
    {
        goto error_unpack;
    ...
    _UNLOCK
    handlerRef = (le_dcs_EventHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);
...
static void Handle_le_dcs_GetTechnology
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Pack the result first
    LE_ASSERT(le_dcs_PackTechnology( &_msgBufPtr, _result ));
...
static void Handle_le_dcs_GetState
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    le_dcs_State_t stateBuffer = (le_dcs_State_t) 0;
    le_dcs_State_t *statePtr = &stateBuffer;
    char interfaceNameBuffer[101] = { 0 };
    ...
    if ( (interfaceNameSize > 100) &&
         (interfaceNameSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting interfaceNameSize from %" PRIuS " to 100", interfaceNameSize);
        interfaceNameSize = 100;
    }
    ...
    if (statePtr)
    {
        LE_ASSERT(le_dcs_PackState( &_msgBufPtr,
                                                      *statePtr ));
    }
    if (interfaceName)
    {
...
static void Handle_le_dcs_Start
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_dcs_Stop
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    uint8_t* _msgBufStartPtr = _msgBufPtr;
    // Declare temporaries for input parameters
    le_dcs_ReqObjRef_t reqRef = NULL;
...
static void Handle_le_dcs_GetReference
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Declare temporaries for input parameters
    char name[33] = {0};
    le_dcs_Technology_t technology = (le_dcs_Technology_t) 0;
    ...
    if (!le_dcs_UnpackTechnology( &_msgBufPtr,
                                               &technology ))
    {
        goto error_unpack;
    }
...
static void AsyncResponse_le_dcs_GetChannels
(
    le_result_t result,
...
)
{
    ...
    {
        LE_FATAL("Handler passed to le_dcs_GetChannels() can't be called more than once");
    }
    ...
    bool channelListResult;
            LE_PACK_PACKSTRUCTARRAY( &_msgBufPtr,
                       ...
                       80, le_dcs_PackChannelInfo,
                       &channelListResult );
    LE_ASSERT(channelListResult);
...
static void Handle_le_dcs_GetChannels
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_dcs_GetChannels ( AsyncResponse_le_dcs_GetChannels, 
        contextPtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_dcs_AddEventHandler :
            Handle_le_dcs_AddEventHandler(msgRef);
            break;
        case _MSGID_le_dcs_RemoveEventHandler :
            Handle_le_dcs_RemoveEventHandler(msgRef);
            break;
        case _MSGID_le_dcs_GetTechnology :
            Handle_le_dcs_GetTechnology(msgRef);
            break;
        case _MSGID_le_dcs_GetState :
            Handle_le_dcs_GetState(msgRef);
            break;
        case _MSGID_le_dcs_Start :
            Handle_le_dcs_Start(msgRef);
            break;
        case _MSGID_le_dcs_Stop :
            Handle_le_dcs_Stop(msgRef);
            break;
        case _MSGID_le_dcs_GetReference :
            Handle_le_dcs_GetReference(msgRef);
            break;
        case _MSGID_le_dcs_GetChannels :
            Handle_le_dcs_GetChannels(msgRef);
            break;
